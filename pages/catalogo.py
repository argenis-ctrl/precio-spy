...
with st.sidebar:
    ...
    if st.button("Refrescar catálogo", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
    st.caption("Actualiza datos desde lasertam.com ahora")
...
st.caption("Productos activos publicados en lasertam.com · Caché 30 min · Usa 'Refrescar' para ver cambios inmediatos")
...
COLS = ["Nombre", "Sesiones", "Precio", "Normal", "Dcto.", "En oferta", "Categorías"]
...
with tab_packs:
    ...
    con_ses  = filtered[filtered["_sessions"].notna()]
    sin_ses  = filtered[filtered["_sessions"].isna()]

    for n in sorted(con_ses["_sessions"].dropna().unique()):
        grupo = con_ses[con_ses["_sessions"] == n]
        lbl   = "1 Sesión" if n == 1 else f"{int(n)} Sesiones"
        with st.expander(f"**Pack {lbl}** — {len(grupo)} productos", expanded=(n in (6, 9))):
            show_table(grupo)

    if not sin_ses.empty:
        with st.expander(f"**Sin sesión definida** — {len(sin_ses)} productos"):
            show_table(sin_ses)

with tab_zonas:
    filtered = apply_filters(df)
    all_cats = sorted({c for cats in filtered["_cats"] for c in cats})

    if not all_cats:
        st.info("Sin categorías con estos filtros.")
    else:
        for cat in all_cats:
            grupo = filtered[filtered["_cats"].apply(lambda x: cat in x)]
            if grupo.empty:
                continue
